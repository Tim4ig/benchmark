...
namespace {
void spmv_csr(const usize* row_ptr, const usize* col_idx, const f32* values, const f32* x, f32* y, usize rows) {
...
BenchResult SpmvAlgorithm::run(const BenchOptions& options) {
  ...
  auto run = [&]() {
    spmv_csr(mat.row_ptr.data(), mat.col_idx.data(), mat.values.data(), x.data(), y.data(), n);
  };
