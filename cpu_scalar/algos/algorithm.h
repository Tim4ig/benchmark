...
template <typename Fn> BenchResult run_benchmark(const BenchOptions& options, Fn&& fn, const BenchmarkSpec& spec) {
  ...
  result.gflops = spec.flops > 0 ? static_cast<f64>(spec.flops) / (timing.avg_ms * 1.0e6) : 0.0;
  result.gbytes = spec.bytes_moved > 0 ? static_cast<f64>(spec.bytes_moved) / (timing.avg_ms * 1.0e6) : 0.0;
  result.checksum = 0.0;
  return result;
}
