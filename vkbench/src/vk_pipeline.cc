...
{
  if (res.descriptor_pool != VK_NULL_HANDLE) { vkDestroyDescriptorPool(ctx.device, res.descriptor_pool, nullptr); }
  if (res.pipeline_layout != VK_NULL_HANDLE) { vkDestroyPipelineLayout(ctx.device, res.pipeline_layout, nullptr); }
  if (res.set_layout != VK_NULL_HANDLE) { vkDestroyDescriptorSetLayout(ctx.device, res.set_layout, nullptr); }
                                            ...
                                            const std::string &spv_path)
{
  ...
  VK_CHECK(vkCreateComputePipelines(ctx.device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr,
                                    &bundle.pipeline));
...
{
  if (pipe.pipeline != VK_NULL_HANDLE) { vkDestroyPipeline(ctx.device, pipe.pipeline, nullptr); }
